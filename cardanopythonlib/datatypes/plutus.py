...
from cardanopythonlib.datatypes.exception import DeserializeException
from cardanopythonlib.datatypes.hash import DATUM_HASH_SIZE, SCRIPT_HASH_SIZE, DatumHash, ScriptHash
from cardanopythonlib.datatypes.nativescript import NativeScript
from cardanopythonlib.datatypes.serialization import (
                ...
                if "constructor" in obj:
                    ...
                    for f, f_info in zip(obj["fields"], fields(cls)):
                        if inspect.isclass(f_info.type) and issubclass(
                            ...
                            and f_info.type.__origin__ is Union # type: ignore
                         ):
                            t_args = f_info.type.__args__ # type: ignore
                            found_match = False
                            for t in t_args:
                                if (
                                    inspect.isclass(t)
...
def script_hash(script: ScriptType) -> ScriptHash:
    ...
    if isinstance(script, NativeScript):
        return script.hash() # type: ignore
